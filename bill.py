...
CLIENT_SECRET_FILE = 'client_secret_837399695209-sel3bf5u78d4apo0b6b2017cco4kp05g.apps.googleusercontent.com.json'
...
def send_mail():
    ...
    lic=Lic_inp.get()
    to = email_inp.get()
    sender = "mystic.unknown19@gmail.com"
...
label_info = tk.Label(top_frame, font = ('arial',40,'bold'), text ="Automatic Event Detection from Traffic Surveillance Video", fg = "blue", bd = 10, anchor = 'w',padx=80)
    ...
    with open('violation.csv', 'r') as csvr:
        ...
        for row in reader:
            if row['LP']==Lic_inp.get():
                ...
                #Make.config(text ='\tVehicle Type: '+(row['make']))
                ...
                url = qrcode.make(''+'\nDEVLOPED BY Traffic Management System CPG 05\n'+'License Plate Number :'+Lic_inp.get()+'\nState : Karnataka' +'\nCity:Bengaluru')
                url.save('code.png')
...
tree = ttk.Treeview(ctr_right, columns=("Image ID", "License_Plate"), selectmode="extended", height=500, yscrollcommand=scrollbary.set, xscrollcommand=scrollbarx.set)
scrollbary.config(command=tree.yview)
scrollbary.pack(side='right')
scrollbarx.config(command=tree.xview)
scrollbarx.pack(side='bottom')
tree.heading('Image ID', text="Image ID")
